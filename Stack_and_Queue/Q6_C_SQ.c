...
void removeUntil(Stack* s, int value)
{
	...
	// Traverse the stack until the chosen value is found
	while (current != NULL && current->item != value) {
		previous = current;
		current = current->next;
	}

	// Pop all elements until the chosen value is found or the stack is empty
	while (s->ll.head != NULL && s->ll.head != current) {
		pop(s);
	}

	// If the chosen value is found, pop it as well
	if (current != NULL && current->item == value) {
		if (previous != NULL) {
			previous->next = NULL;  // Disconnect the stack after the chosen value
		}
		pop(s);  // Pop the chosen value
	}
/* add your code here */
/*
...
for loop can break like while loop, remeber to consider s->|.Size == 0
The function prototype is given as follows:
void removeuntilstack(Stack *s, int value);
Given a stack (1, 2, 3, 4, 5, 6, 7) with the topmost number displayed on the left, calling removeUntilStack () with value = 4 will produce the stack (4, 5, ¬Ò, 7).
