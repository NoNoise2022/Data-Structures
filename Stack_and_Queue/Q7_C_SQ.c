...
int balanced(char* expression)
{
	...
	while (*expression != '\0') {
		...
		if (current == '(' || current == '[' || current == '{') {
			...
			if (isEmptyStack(&s)) {
				return 1;  // Unbalanced: closing symbol encountered without corresponding opening symbol
			}
			...
			if ((current == ')' && top == '(') || (current == ']' && top == '[') || (current == '}' && top == '{')) {
				...
				return 1;  // Unbalanced: mismatched symbols
			}
		}
	...
	// If stack is not empty at the end, expression is unbalanced
	if (!isEmptyStack(&s)) {
		return 1;  // Unbalanced: unmatched opening symbols remaining
	}
...
The function prototype is given as follows: int balanced (char *expression);

2: Check whether expressions comprised of the characters ()Ill}
is balanced:
0: Quit:
Please input your choice (1/0): 1
Enter expressions without spaces to check whether it is balanced or not: {@1]}
{0011} balanced!
...
1: Enter a string:
2: Check whether expressions comprised of the characters (>[11}
is balanced:
0: Quit:
Please input your choice (1/0): 1
Enter expressions without spaces to check whether it is balanced or not: [ (1{)])
